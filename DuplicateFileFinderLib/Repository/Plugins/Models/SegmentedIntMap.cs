// DuplicateFileFinderLib/Repository/Plugins/Models/SegmentedIntMap.cs
...
/// <summary>
/// An int->T map optimized for fast (de)serialization:
...
public sealed partial class SegmentedIntMap<T>
    where T : struct
{
    /// <summary>
    /// Sorted by StartId ascending.
    /// </summary>
    public required MapSegmentInt<T>[] Segments { get; init; } = [];

    [MemoryPackIgnore]
    public int SegmentCount => Segments.Length;

    public static SegmentedIntMap<T> Empty { get; } = new() { Segments = [] };
    ...
    /// </summary>
    public static SegmentedIntMap<T> Build(
        ...
        int gapThreshold = 64)
    {
        ...
        var segments = new List<MapSegmentInt<T>>(capacity: Math.Min(1024, sorted.Length));
        ...
        for (int i = 1; i < sorted.Length; i++)
        {
            ...
            if (gap > gapThreshold)
            {
                segments.Add(MapSegmentInt<T>.BuildSegment(sorted, segFirstIndex, i - 1, segStart, segEnd));
                segFirstIndex = i;
                segStart = k;
                segEnd = k;
            }
        ...
        segments.Add(MapSegmentInt<T>.BuildSegment(sorted, segFirstIndex, sorted.Length - 1, segStart, segEnd));

        return new SegmentedIntMap<T> { Segments = segments.ToArray() };
    }

    /// <summary>
    /// Convenience: build from a Dictionary without changing callers.
    /// </summary>
    public static SegmentedIntMap<T> FromDictionary(
    ...
    private int FindSegmentIndexForKey(int key)
    {
