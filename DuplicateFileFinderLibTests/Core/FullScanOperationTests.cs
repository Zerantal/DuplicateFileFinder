...
public class FullScanOperationTests
{
    [Fact]
    public async Task ExecuteAsync_UsesRotationalVolumeInfo_SetsDegreeOfParallelismTo1()
    {
        ...
        // Make sure the root does NOT exist so we short-circuit before hashing
        var nonExistingPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString());

        repo.BeginScan(nonExistingPath, ScanOperation.FullScan, vinfo).Returns(session);

        var op = new FullScanOperation(host, fs, pipeline, volProvider);
        ...
        // Assert
        // We cannot directly read the private _hashDegreeOfParallelism from here,
        // but we *can* assert that VolumeInfo was queried and BeginScan saw the same vinfo.
        volProvider.Received(1).GetVolumeInfoForPath(nonExistingPath);
        repo.Received(1).BeginScan(nonExistingPath, ScanOperation.FullScan, vinfo);
    }
    ...
    public async Task ExecuteAsync_CancellationDuringEnumeration_PropagatesAndMarksScanCancelled()
    {
        ...
        // Use a real directory so Directory.Exists == true
        var tempDir = Directory.CreateDirectory(
            Path.Combine(Path.GetTempPath(), "dff_fullscan_cancel_" + Guid.NewGuid()));

        var vinfo = (VolumeInfo?)null;
        volProvider.GetVolumeInfoForPath(tempDir.FullName).Returns(vinfo);

        repo.BeginScan(tempDir.FullName, ScanOperation.FullScan, vinfo).Returns(session);
        repo.GetRepoView().Returns(repoView);

        // Root dir record used by EnumerateFullAsync
        ...
        // Assert: cancellation branch should mark scan as cancelled
        await session.Received(1)
